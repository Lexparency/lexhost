...
BOT_LIST = ['SemrushBot', 'ltx71', 'bingbot', 'PetalBot', 'Bytespider',
            'Googlebot', 'DotBot', 'SeznamBot', 'Applebot', 'MJ12bot', 'Adsbot',
            'adscanner', 'UptimeRobot', 'Barkrowler', 'CCBot', 'YandexBot',
            'SemanticScholarBot', 'Seekport Crawler', 'facebookexternalhit',
            'VelenPublicWebCrawler', 'centuryb.o.t9', 'SEOkicks', 'serpstatbot.com',
            'AhrefsBot', 'BLEXBot', 'Metrics Tools Analytics Crawler', 'Neevabot',
            'Amazonbot', 'Mediapartners-Google', 'InfoTigerBot', 'GrapeshotCrawler',
            'DataForSeoBot', 'Exabot', 'MegaIndex.ru']

IGNOREIPS = {
    '167.86.114.97',  # sleipnir
    '127.0.0.1'
}
...
class LogEntry:
    ...
    __slots__ = ['ip_address', 'timestamp', 'method', 'path', 'protocol',
                 'response_code', 'some_number', 'referrer', 'client']
...
class LogFile(list):
    ...
    @property
    def bot_report(self):
        botquests = [r for r in self
                     if r.bot_name is not None and r.ip_address not in IGNOREIPS]
        faileds = [f'{r.response_code}, {r.bot_name}'
                   for r in botquests if r.response_code not in (200, 301, 308, 307)]
        cf = Counter(faileds)
        c = Counter([r.bot_name for r in botquests])
        return {
            'counts': [{'name': name, 'count': count}
                       for name, count in c.items()],
            'faileds': [{'name': name, 'count': count}
                        for name, count in cf.items()],
        }

    @property
    def user_report(self):
        u_requests = [r for r in self if r.bot_name is None
                      and r.is_main_request and r.ip_address not in IGNOREIPS
                      and r.ip_address not in JAILEDS]
        faileds = [f'{r.path}, {r.response_code}, {r.referrer}'
                   for r in u_requests if r.response_code != 200
        ...
        return {
            ...
            'referrers': [
                ...
                for referrer, count in sorted(referrer_scores.items(),
