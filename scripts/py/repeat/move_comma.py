...
def cleanse_title(previous):
    ...
    previous = previous.replace(
        '<p class="lxp-title_essence"> )', ') <p class="lxp-title_essence">'
    )
    previous = previous.replace(
        ' <p class="lxp-title_essence">)', ') <p class="lxp-title_essence">'
    )
    ...
    for initial in (f" ({text_with_relevance})</p>", f" {text_with_relevance}</p>"):
        previous = previous.replace(initial, f"</p> ({text_with_relevance})")
    previous = previous.replace(f" {text_with_relevance}", f" ({text_with_relevance})")
    return previous
...
def fix_cover_anchors(c: dm.Cover):
    ...
    for a_name in dm.Cover.iter_anchors():
        ...
        for anchor in anchors:
            href = anchor.href.replace("/TXT/HTML/?uri=CELEX:", "/ALL/?uri=CELEX:")
