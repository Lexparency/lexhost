...
class DocumentVersion:
    ...
    def to_dict(self) -> dict:
        return {es_doc.meta.id: es_doc.to_dict()
    ...
    def get(cls, domain, id_local, version):
        ...
        return cls(
            ...
            definitions=[dm.Definition.get(global_id)
                         for sub_id, global_id in stg.items()
...
class DocumentHistory(dm.VersionsMap):
    ...
    def sub_to_global_id(self, version):
        result = {
            item.sub_id: '-'.join((
                self.meta.id, item.sub_id, item.hidden_version))
    ...
    def get_latest_loaded(self):
        ...
        return {
            ...
            for part in DocumentVersion.get(
                self.domain, self.id_local, latest_available).iter_parts()}
    ...
    def incorporate(self, doc: DocumentVersion):
        ...
        latest = self.latest
        self.append_availability(dm.VersionAvailability(
            version=doc.version,
        ...
        for new_part in doc.iter_parts():
            ...
            if new_part.sub_id != 'COV' \
                    or latest == self.latest_available or latest is None:
                latest_loaded_part = latest_loaded.get(new_part.sub_id)
            else:
                latest_loaded_part = dm.Cover.get(
                    self.sub_to_global_id(latest)['COV'])
            new_part.abstract.is_latest = True
            if latest_loaded_part is None:
                # Case: the new doc has a completely new part.
                new_part.save()
                self.exposed_and_hidden.append(dm.ExposedAndHiddenVersions(
                ...
                self.hidden_to_exposed[new_part.sub_id][latest_loaded_part.version] \
                    .append(doc.version)
        ...
        if availables and self.availabilities[-1].available:
            ...
            for sub_id in obsoletes:
                ...
                if part.abstract.in_force is False \
                        and part.abstract.is_latest is False:
                    continue
    ...
    def insert_unavailable(self, version, date_document, after=None):
        ...
        """
        if after is None:
            self.availabilities.append(dm.VersionAvailability(
                version=version,
            ...
            for i, availability in enumerate(self.availabilities):
                ...
                raise NotFoundError(
                    f'Document {self.id_local} with version {after}'
                    f' does not exist.')
            self.availabilities.insert(
                i + 1,
                dm.VersionAvailability(version=version,
                                       date_document=date_document,
                                       available=False))
        self.save()
    ...
    @property
    def in_force(self):
        values = set(hit.abstract.in_force for hit in self.iter_atoms()
                     if self.latest_available in hit.abstract.version)
        if len(values) == 1:
            return values.pop()
        else:
            raise ValueError(f"Document {self.domain}-{self.id_local} has "
    ...
    def in_force(self, value):
        ...
        ids = [hit.meta.id for hit in self.iter_atoms()
