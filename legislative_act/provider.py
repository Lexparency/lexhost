...
class DocumentProvider(Clearable):
    ...
    def basic_error_msg(self):
        return (
            f"Could not find fragment {repr(self.domain)}" f", {repr(self.id_local)}."
        )
    ...
    def get_snippet(self, fragment_id, version, snippet):
        ...
        if fragment_id == "TOC":
            ...
            return render_template(
                "fragments/toc_snippet.html", nodes=nodes, title=self.title
            )
        ...
        if snippet == "0":
            ...
            try:
                out_element.append(
                    deepcopy(article.xpath(f'(.//li[@id="{snippet}"])[1]')[0])
                )
            except IndexError:
                article_snippet()
            else:
    ...
    def search(self, words, *, page=1, all_versions=False):
        searcher = DocumentSearcher(
            words, self.domain, self.id_local, all_versions=all_versions
        )
