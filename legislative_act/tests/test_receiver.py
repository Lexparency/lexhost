...
class TestReceiver(TestCase):
    ...
    def setUpClass(cls):
        with open(
                os.path.join(cls.DATA_PATH, 'document_1.html'), mode='r') as f:
            cls.received_document = DocumentReceiver.relaxed_instantiation(
                f.read(),
                logger=Mock())
        with open(
                os.path.join(cls.DATA_PATH, 'document_1.json'), mode='r') as f:
            expected = convert_datetime_patterns(json.load(f))
        cls.expected = {doc['_id']: doc['_source'] for doc in expected['data']}
        with open(os.path.join(cls.DATA_PATH,
                               'document_1_flat.json'), mode='r') as f:
            expected_flat = convert_datetime_patterns(json.load(f))
        cls.expected_flat = {doc['_id']: doc['_source']
                             for doc in expected_flat['data']}
    ...
    def test_receive(self):
        ...
        for key in received.keys():
            ...
            expected, actual = ignore_order(
                self.expected[key], received[key])
    ...
    def test_stub(self):
        with open(os.path.join(self.DATA_PATH, 'stub1.html'), mode='r') as f:
            stub = DocumentReceiver.relaxed_instantiation(
                f.read(), logger=Mock())
        ...
        with open(os.path.join(self.DATA_PATH, 'stub1_cover_flat_dict.json'),
                  mode='r') as f:
...
class MinorTests(TestCase):
    ...
    in_2_out = [('#ART_201', '/eu/dummy/ART_201/'),
    ...
    def setUp(self):
        self.source = et.Element('article', {'id': 'ART_200',
                                             'class': 'lxp-article'})
