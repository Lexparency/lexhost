...
class TestHistory(TestBase):
    def setUp(self):
        super().setUp()
        with open(os.path.join(self.DATA_PATH, "document_1.html"), mode="r") as f:
            self.document_1 = DocumentReceiver.relaxed_instantiation(
                f.read(), logger=Mock()
            )
        with open(os.path.join(self.DATA_PATH, "document_1a.html"), mode="r") as f:
            self.document_1a = DocumentReceiver.relaxed_instantiation(
                f.read(), logger=Mock()
            )
        with open(
            os.path.join(self.DATA_PATH, "document_1_versions.json"), mode="r"
        ) as f:
            self.versions_1 = DocumentHistory.from_es(json.load(f))
        with open(
            os.path.join(self.DATA_PATH, "document_1_1a_versions.json"), mode="r"
        ) as f:
    ...
    def test_incorporate(self):
        ...
        self.assertEqual(
            sort_exposed_and_hidden(self.versions_1_1a.to_dict()["exposed_and_hidden"]),
            sort_exposed_and_hidden(dh.to_dict()["exposed_and_hidden"]),
        )
        self.assertEqual(
            self.versions_1_1a.to_dict()["availabilities"],
            dh.to_dict()["availabilities"],
        )
        for sub_id, is_latest in (("TOC", False), ("ART_1", False), ("ART_3", False)):
            part = dm.Base.get(
                index=dm.index_name,
        ...
        expectation = [
            {
                ...
                "date_received": get_today(),
                "available": True,
            },
            {
                "version": "20171222",
                "date_document": date(2017, 12, 22),
                "date_received": get_today(),
                "available": False,
            },
            {
                "version": "20171224",
                "date_document": date(2017, 12, 24),
                "date_received": get_today(),
                "available": True,
            },
        ]
    ...
    def test_purge(self):
        ...
        self.assertRaises(
            NotFoundError, dm.VersionsMap.get, "{}-{}".format(dh.domain, dh.id_local)
        )
...
class TestStub(TestBase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(TestHistory.DATA_PATH, "stub1.html"), mode="r") as f:
            self.stub_1 = DocumentReceiver.relaxed_instantiation(
                f.read(), logger=Mock()
            )
        with open(os.path.join(TestHistory.DATA_PATH, "stub1.json"), mode="r") as f:
    ...
    def test_save_stub(self):
        self.stub_1.save()
        self.assertEqual(
            *ignore_order(self.cover, dm.Cover.get("eu-stub1-COV-initial").to_dict())
        )
        self.assertEqual(
            *ignore_order(self.versions_map, dm.VersionsMap.get("eu-stub1").to_dict())
        )
    ...
    def test(self):
        for version in ("initial", "20190101", "20200101"):
            with open(os.path.join(self.DATA_PATH, f"{version}.html")) as f:
                dr = DocumentReceiver.relaxed_instantiation(f.read(), logger=Mock())
                dr.save()
...
class TestConsistentRelation(TestArticleAbrogation):
    def setUp(self):
        super().setUp()
        with open(
            os.path.join(self.DATA_PATH, os.pardir, "amends_breg.html"),
            encoding="utf-8",
        ) as f:
            DocumentReceiver.relaxed_instantiation(f.read()).save()
    ...
    def test(self):
        ...
        self.assertEqual(
            [
                dm.Anchor(
                    href="https://lexparency.org/eu/ambreg/",
                    text="Reg. 0815a",
...
class TestConsistentRelation2(TestArticleAbrogation):
    def setUp(self):
        ...
        c.amended_by = [
            dm.Anchor(
                href="https://lexparency.org/eu/ambreg0/",
                text="Reg. 0815",
