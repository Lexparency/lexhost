...
def _node_to_element_html(node: doc.ContentsTableNode) -> et.ElementBase:
    try:
        display_text = SEPARATOR.join((node.heading.ordinate,
                                       node.heading.title))
...
class ContentsTable(doc.ContentsTable):
    ...
    def leaf_to_neighbour(self) -> dict:
        ...
        def node_2_neighbour(n: doc.ContentsTableNode):
            return {'ordinate': n.heading.ordinate,
                    'id': n.locator}

        leaf_to_node = [node for node in self.table
        ...
        result.update({
            node.locator: {'left': node_2_neighbour(leaf_to_node[k-1]),
    ...
    def toccordior(self, version, document_id=None, method='html') -> et.ElementBase:
        ...
        if method == 'html':
            toccordion = et.Element('ul', {'id': 'toccordion',
                                           'class': 'w3-ul w3-medium'})
        else:
            assert method == 'xml'
    ...
    def ultimate_parents(self):
        ...
        for node in self.table:
            ...
            if type(node.children[0]) == doc.ContentsTableNode:
                ...
                node.children = [l2n.pop(locator)
                                 for locator in node.children]
