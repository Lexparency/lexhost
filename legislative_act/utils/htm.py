...
def textify(element: et.ElementBase, with_tail=True, simplify_blanks=False):
    ...
    if simplify_blanks:
        ...
        return et.tostring(element, method='text',
                           encoding='unicode', with_tail=with_tail).strip()


def strip_html(element_string):
    element = et.fromstring('<div>{}</div>'.format(element_string),
...
def inner_tostring(element, skip_ns_declarations=False,
                   simplify_blanks=False, **kwargs):
    def do_skips(res: str) -> str:
        declarations = [" xmlns:{}=\"{}\"".format(key, value)
    ...
    result = ('' if is_negligible(element.text) else escape(element.text))\
             + ''.join([et.tostring(sub_element, encoding='unicode', **kwargs)
