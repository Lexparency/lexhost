...
class SimpleHistory:
    ...
    def __repr__(self):
        return f"SimpleHistory({self.domain}, " \
            f"{self.id_local}, {self.availabilities})"
    ...
    def to_dict(self):
        return {
            ...
            'versions': [popped(a.to_dict(), 'date_received')
...
class HistoryList(BasicSearcher):
    ...
    def search(self):
        result = dm.Search().params(track_total_hits=True) \
    ...
    def display_hit(self, hit) -> dict:
        try:
            ...
            try:
                ...
                latest_cover_id = None
        return SimpleHistory(*hit.meta.id.split('-'),
                             availabilities=availabilities,
                             latest_cover_id=latest_cover_id).to_dict()
