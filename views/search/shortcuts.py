...
class TitledLocation(namedtuple('TL', ['href', 'title'])):
    ...
    def get_cover_display_dictionary(self):
        return display_dictionary(Cover.get(
            f'{self.domain}-{self.id_local}-COV-initial'))
...
class DeadSimpleShortCut:
    ...
    @property
    def locations(self):
        return sorted({TitledLocation(path, None) for path in self.paths},
                      reverse=True, key=attrgetter('href'))


def get_abbreviations():
    file_path = os.path.join(os.path.dirname(lexref.__file__),
                             'static', 'named_entity.csv')
    with open(file_path, encoding='utf-8') as f:
        lines = [line.strip().split(',') for line in f.readlines()]
    assert lines[0] == ['tag', 'language', 'title_pattern',
                        'abbreviation', 'title']
    return [line[3] for line in lines
            if line[1].upper() == LANG and line[3] != '']
...
class ShortCutter:
    ...
    @property
    def hits(self):
        references = DeadSimpleShortCut(self.search_words, self.context) \
            .locations
