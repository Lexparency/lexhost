...
class SearchForm(dict):
    slots = ('search_words', 'type_document', 'deep', 'in_force',
    ...
    def parse(cls, get_dict: MultiDict):
        ...
        if len(get_dict) == 0:
            return None
        query = {'type_document':
                 get_dict.getlist('type_document')}
    ...
    def as_filter(self):
        ...
        return Filter(**{key: convert(key, value)
    ...
    def form_config(self):
        return {
            ...
            'type_document': {
                t: 'checked' * (t in self.get('type_document', []))
                for t in ACT_TYPES
            },
            'deep': {
