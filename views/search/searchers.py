...
def search_document(path, domain, id_local, words, page, all_versions=False):
    ...
    result["pages"] = SearchForm(search_words=words).repaginator(result["pages"], path)
    content = {
        "search_words": words,
        "messenger": standard_messages,
        "cover": cover,
        "document_title": cover.get("pop_title") or cover.get("title"),
        "title": '{} "{}"'.format(standard_messages["search_results_for"], words),
...
def search_corpus(path: str, args: MultiDict):
    ...
    if query is not None:
        ...
        try:
            title = '{} "{}"'.format(
                standard_messages["search_results_for"], query["search_words"]
            )
        except KeyError:
