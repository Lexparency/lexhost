...
class SitePlace(namedtuple("SitePlace", ["loc", "lastmod", "changefreq", "priority"])):
    def to_xml(self):
        url = et.Element("url")
...
class SiteMap(list):
    ...
    def get_site_place(self, id_local) -> SitePlace:
        ...
        return SitePlace(
            ...
            priority=cover.score_multiplier / 1000.0,
        )
    ...
    def build(cls, domain):
        ...
        for item in get_featured(self.domain, FEATURED[self.domain]):
            ...
            try:
                ...
                self.logger.error(f"Something went wrong for {id_local}", exc_info=True)
        return self
