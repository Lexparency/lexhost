...
def _get_nationals(domain, id_local, sub_id):
    ...
    for h in sorted(
        s.scan(), key=lambda x: ref_sort_key(x, sub_id is None), reverse=sub_id is None
    ):
    ...
    return [
        {"country_name": country, "targets": targets}
...
def get_nationals(domain, id_local, sub_id=None):
    ...
    return {
        "cover": _get_nationals(domain, id_local, None),
...
def write_nationals(target, url, text, country_name, title=None):
    ...
    try:
        ...
        nr = dm.NationalReference(
            country_name=country_name, text=text, urls=[url], references=[target]
        )
