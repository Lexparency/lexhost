...
def snippet(domain, document_id, fragment_id, version, snip):
    return DocumentProvider.get(domain, document_id).get_snippet(
        fragment_id, version, snip
    )
...
class Read(Clearable, metaclass=Singleton):
    ...
    def article(self, document_domain, document_id, sub_id, version):
        ...
        try:
            base["cover"]["in_force"] = base["cover"]["in_force"] and (
                (hidden_version == dp.current) or article["in_force"]
            )
        except KeyError:
            pass
        content = dict(
        ...
        if sub_id == "PRE":  # Make Contentstable the overview.
            content["neighbours"]["left"] = {
                "ordinate": standard_messages["TOC_ordinate"],
                "id": "TOC",
            }
        content["title"] = get_title_article(
            article["ordinate"],
    ...
    def overview(self, domain, id_local, version):
        ...
        content["title"] = get_title_overview(
            base["cover"].get("pop_acronym"),
            base["cover"].get("pop_title"),
            base["cover"].get("id_human"),
    ...
    def _document_id_gone(document_id):
        ...
        if document_id.startswith("Regulation_") or document_id.startswith(
            "Directive_"
        ):
    ...
    def __call__(self, domain, document_id, sub_id=None, version=None):
        ...
        forwarding = (version == "latest") or (
            sub_id == "TOC" and version in ("latest", None)
        )
        ...
        if "snippet" in request.args:
            return make_response(
                self.snippet(domain, id_local, sub_id, version, request.args["snippet"])
            )
        if sub_id != "TOC" and version == "latest":
            dp = dp_get(domain, id_local)
            leaf_versions = dp.get_leaf_versions(sub_id)
            if not leaf_versions:
                raise NotFound(f"Could not find {domain}, {id_local}, {sub_id}.")
        ...
        if forwarding:
            ...
            return redirect(target_url, code=self.get_redirect_code(r_version, version))
    ...
    def get_redirect_code(request_version, target_version):
        if target_version == "latest":
            return 301
        elif request_version in (None, "latest") and request_version != target_version:
            return 307
        return 301
    ...
    def get_paths(self, domain, document_id, sub_id, version):
        return {
            "canonical_path": self.get_canonical_path(domain, document_id, sub_id),
            "standard_path": self.get_standard_path(
                domain, document_id, sub_id, version
            ),
            "int_canonical_path": self.get_int_canonical_path(
                domain, document_id, sub_id
            ),
        }
