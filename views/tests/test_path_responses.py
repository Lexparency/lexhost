...
class TestViews(ViewsTester):
    ...
    def setUp(self):
        super().setUp()
        self.maxDiff = None
        with open(os.path.join(DATA_PATH, 'path_2_snippet.json'), encoding='utf-8') as f:
            data = json.load(f)
        self.path2snippet = {i['path']: i['snippet']
                             for i in data['path_2_snippet']}
        ...
        with open(os.path.join(DATA_PATH, 'sitemap.xml'), encoding='utf-8') as f:
            self.expected_sitemap = f.read() \
                .replace('2020-02-16', date.today().strftime('%Y-%m-%d'))
    ...
    def test_redirects(self):
        for entry_url, expected_url, status_code in url_to_redirect:
            ...
            self.assertEqual(
                expected_url, relocation, f'Failed for {entry_url}')
            self.assertEqual(
    ...
    def test_snippet(self):
        for path, snippet in self.path2snippet.items():
            self.assertEqual(snippet,
                             self.client.get(path).data.decode('utf-8'))
        for path in ("/eu/32013R0575/ART_9v/?snippet=z", "/eu/CHAR/?snippet=0"):
            r = self.client.get(path)
            self.assertEqual(r.status_code, 404)

    search_hit_tag = \
        '<div id="search-hit-{0}" class="w3-container search-hit">'.format
    ...
    def test_corpus_search(self):
        response = self.client.get(
            '/eu/search?search_words=prudential')
    ...
    def test_stubbed(self):
        ...
        self.assertTrue(
            b'Unfortunately, this document is not yet available here.'
            in response.data)
...
class TestDiamonds(ViewsTester):
    ...
    botapi = {}
    for fn in ('botapi_recents.json', 'botapi_document_crr.json',
    ...
    def setUpClass(cls):
        super().setUpClass()
        with open(os.path.join(DATA_PATH, '32013R0575-initial.html'),
                  encoding='utf-8') as f:
            DocumentReceiver.relaxed_instantiation(f.read()).save()
    ...
    canonical_2_entry = {
        '/eu/CRR/': ['/eu/32013R0575/', '/eu/crr/', '/eu/32013R0575/TOC/',
                     '/eu/32013R0575/TOC/latest'],
        '/eu/CRR/ART_2/': ['/eu/32013R0575/ART_2/', '/eu/32013R0575/art_2/',
...
class TestRecentsSingleStep(ViewsTester):
    ...
    def setUp(self):
        super().setUp()
        with open(os.path.join(DATA_PATH, 'botapi_single_step.json'), encoding='utf-8') as f:
