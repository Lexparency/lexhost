...
def extract_amends(in_text):
    ...
    if m is None:
        return
    r = get('https://reflector.lexparency.org/reflect',
            headers={"Content-type": "application/json"},
            data=json.dumps({"language": LANG_2.upper(),
...
class LatestHistories(BasicSearcher):
    ...
    def search(self):
        return Search().query(
            ...
            path='availabilities',
            query=Q('range',
                    availabilities__date_document={"gte": self.date_from,
                                                   "lte": date.today()})
        )
    ...
    def display_hit(self, hit):
        ...
        try:
            result['date_document'] = sorted([
                a['date_document'] for a in hit['availabilities']
                if a['available']])[-1]
...
class DocumentBot:
    ...
    def __init__(self, domain, doc_id):
        ...
        self.dv = DocumentVersion.get(domain, self.id_local,
                                      self.dh.latest_available)
        self.dv.toc = ContentsTable.get(self.dv.toc.meta.id)
        self.d_url = DEFAULT_IRI + f'/{domain}/{doc_id}/'
        self.d_name = document_name(self.dv.cover.id_human,
                                    self.dv.cover.pop_acronym,
                                    self.dv.cover.abstract.id_local)

    def iter_implemented_by(self):
        changes = {
            a.href.strip('/').split('/')[-1] for a in self.dv.cover.amends
            if a.href.startswith(DEFAULT_IRI)
        }
        s = Search().filter('term', doc_type='cover') \
